...
router.post(
  "/signup",
  uploadCloudinary.single("defaultImg"),
  (req, res, next) => {
    // set variables to information from form
    ...
    User.findOne({ $or: [{ email: userEmail }, { username: userName }] })
      .then(foundUser => {
        ...
        // create user based on form using predefinded variables
        User.create({
          email: userEmail,
          ...
          .then(user => {
            ...
            req.logIn(user, err => {
              if (err) {
                ...
                req.flash(
                  "error",
                  "Auto login is not working, please login manually"
                );
                res.redirect("/login");
                return;
              }
              // if okay to log in redirect to profile page
